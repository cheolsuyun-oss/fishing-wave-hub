...
function ultraBaseTime() {
  ...
  const baseDt = `${yyyy}-${mm}-${dd}T${String(bh).padStart(2, "0")}:30:00+09:00`;
  return {
    date: yyyy + mm + dd,
...
async function collectPoint(point: { code: string; nx: number; ny: number }) {
  const { date, time, baseDt } = ultraBaseTime();
  ...
  try {
    ...
    console.log("ULTRA JSON PARSE ERROR for", point.code, text.slice(0, 200));
    return;
  }
  ...
  if (error) {
    console.log(point.code, "ULTRA ERROR:", error.message);
  } else {
    console.log(point.code, "ULTRA OK", rows.length, "rows", baseDt);
  }
}

async function collectChunk(chunk: { code: string; nx: number; ny: number }[]) {
  await Promise.all(chunk.map(p => collectPoint(p).catch(e =>
    console.error(p.code, "ultra failed:", e instanceof Error ? e.message : String(e))
  )));
}
